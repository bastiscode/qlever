...
std::optional<ad_utility::MaybeOwnedVector> SplitVocabulary<
  ...
  // Visit the underlying vocabulary and retrieve the requested embedding vector
  // if it is an `EmbeddingVocabulary`.
  const auto& vocab = underlying_[getMarker(indexWithMarker)];
  return std::visit(
...
bool SplitVocabulary<SF, SFN, S...>::isEmbeddingAvailable() {
  // If any of the underlying vocabularies is an `EmbeddingVocabulary`, then this
  // `SplitVocabulary` is able to provide embeddings.
