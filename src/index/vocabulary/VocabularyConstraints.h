...
// here because in a combined `SplitVocabulary` (`regular | geo | embedding`) the
// `getGeoInfo` dispatch hits the embedding sub-vocabulary in its `else` branch,
// whose `static_assert` requires this concept.
template <typename T>
