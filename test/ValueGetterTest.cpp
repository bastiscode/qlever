...
// The serialized datatype suffix of an `fp32-vector` literal, used to build test
// literals.
...
TEST(EmbeddingValueGetterTest, OperatorWithVocabIdLocalVocabAndLiteral) {
  EmbeddingTester t;
  // A valid `fp32-vector` literal decodes to its float vector — across the
  // normal vocabulary (sidecar fast-path or literal fallback), the local vocab,
  // and a literal passed directly to the getter.
  t.checkFromLocalAndNormalVocabAndLiteral(
...
TEST(EmbeddingValueGetterTest, MalformedVectorLiteralIsRejected) {
  ...
  for (std::string_view body : {"\"[]\"", "\"[1, ]\"", "\"[1, x]\"", "\"abc\""}) {
